...
class SnailfishNumber:
    ...
    def explode(self):
        ...
        number_prev = re.sub(
            r"(.*\D)(\d+)",
            lambda m: m.group(1) + str(int(m.group(2)) + prev),
            number_prev,
            count=1,
        )
        # replace first integer
        number_following = re.sub(
            r"\d+", lambda m: str(int(m.group()) + following), number_following, count=1
        )
        new_number = number_prev + "0" + number_following
        changed = new_number != self.number
        self.number = new_number
        if changed:
            print(f"explodedOn--{pair}-- {self.number}")
        return changed
...
if __name__ == "__main__":
    with open("test/day18.txt", "r") as file:
    ...
    for number in numbers_to_add[1:]:
        s_number += number
        print("ADDED NEW NUMBER", s_number)
        s_number.reduce()
